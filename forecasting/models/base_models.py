...
class ForecastModel(models.Model):
    ...
    MODEL_TYPES = [
        ...
        ('randomforest', 'Random Forest'),  # Agregar alias
...
class ForecastAccuracy(models.Model):
    ...
    def __str__(self):
        return f"Accuracy {self.forecast.product.sku} - {self.percentage_error:.2f}%"
    
    def save(self, *args, **kwargs):
        """Calcula métricas automáticamente"""
        if self.forecast and self.actual_demand is not None:
            ...
            # Error porcentual
            if actual != 0:
                self.percentage_error = (self.absolute_error / actual) * 100
            else:
                self.percentage_error = 0
            
            # Error cuadrático
            self.squared_error = (predicted - actual) ** 2
            
            # Sesgo
            self.bias = predicted - actual
...
class ReorderRecommendation(models.Model):
    ...
    def days_until_stockout(self):
        ...
        if self.expected_stockout_date:
            ...
            delta = self.expected_stockout_date - date.today()
    ...
    def is_urgent(self):
        ...
        days_until = self.days_until_stockout
        return days_until is not None and days_until <= 7
