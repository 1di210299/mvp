...
class LinearRegressionForecaster(BaseForecaster):
    ...
    def __init__(self, hyperparameters: Optional[Dict[str, Any]] = None):
        ...
        default_params = {
            ...
            'seasonal_periods': [7, 30],  # Períodos estacionales a considerar
    ...
    def _prepare_features(self, data: pd.DataFrame, target_column: str) -> pd.DataFrame:
        ...
        if self.hyperparameters.get('polynomial_degree', 1) > 1:
            if self.poly_features is None:
                ...
                sample_size = min(100, len(all_features))
                if sample_size > 0:
    ...
    def fit(self, data: pd.DataFrame, target_column: str = 'quantity') -> 'LinearRegressionForecaster':
        """
        ...
        try:
            ...
            for i, future_date in enumerate(future_dates):
                try:
                    ...
                    # Rellena el valor futuro temporalmente con el último valor conocido
                    ...
                    if features is not None and not features.empty and future_date in features.index:
                        ...
                        # Si no se pueden crear features válidas, usa el último valor
            ...
            try:
                train_features = self._prepare_features(self.training_data, target_column)
                if train_features is not None and not train_features.empty:
                    ...
                    residuals_std = np.std(self.training_data[target_column].loc[train_features.index] - train_predictions)
                else:
                    residuals_std = np.std(predictions) * 0.1  # Fallback
            except Exception:
                residuals_std = np.std(predictions) * 0.1 if predictions else 1.0
