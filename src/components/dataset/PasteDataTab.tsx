...
import ConnectApiDbTab from './ConnectApiDbTab';
...
const AddDatasetModal: React.FC<AddDatasetModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [pastedDataInfo, setPastedDataInfo] = useState<any>(null);
  
  if (!isOpen) return null;

  const handleFileSelect = (file: File) => {
    setSelectedFile(file);
    // Simulación: Generar columnas de muestra basadas en el archivo
    const mockColumns = [
      { name: 'id', type: 'number' },
  ...
  const handleConnectionSetup = (config: any) => {
    setConnectionConfig(config);
    // Simulación: Generar columnas de muestra basadas en la conexión
    const mockColumns = [
  ...
  const handleDataPaste = (data: any) => {
    setPastedDataInfo(data);
    // Simulación: Generar columnas de muestra basadas en los datos pegados
    const mockColumns = [
  ...
  const handleSave = () => {
    // Crear dataset object con toda la información recolectada
    const datasetInfo = {
      name: datasetName,
      description,
      category,
    ...
    setPastedDataInfo(null);
  };

  // Función para renderizar el icono del paso actual
  ...
  const renderStepIndicator = () => {
    return (
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex-1 h-0.5 mx-2 bg-cyber-detail">
          <div className={`h-full bg-cyber-cyan ${
            currentStep === 'advanced' || currentStep === 'confirm' ? 'w-full' : 'w-0'
          } transition-all duration-300`}></div>
        </div>
  ...
  // Renderiza el paso de carga
  const renderUploadStep = () => {
    return (
      <div className="space-y-6">
        <div className="space-y-2">
          <label className="block text-sm font-medium text-cyber-text">Dataset Name*</label>
          <input
            type="text"
            value={datasetName}
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-cyber-text">Description</label>
          <textarea
            value={description}
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-cyber-text">Category</label>
          <select
            value={category}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            >
              ...
              <span className="text-xs mt-1 opacity-70">CSV, Excel, JSON, Parquet</span>
            </button>
          ...
          {uploadMethod === 'file' && <UploadFileTab onFileSelect={handleFileSelect} />}
          {uploadMethod === 'api' && <ConnectApiDbTab onConnectionSetup={handleConnectionSetup} />}
          {uploadMethod === 'paste' && <PasteDataTab onDataPaste={handleDataPaste} />}
        </div>
      </div>
    );
  };

  // Renderiza las configuraciones del paso siguiente (igual que antes)
  const renderConfigureStep = () => {
    // Mismo código que compartiste en la versión anterior
    return (
      <div className="space-y-6">
        <div>
                  ...
                  {[...Array(3)].map((_, rowIndex) => (
                    <tr key={rowIndex}>
                      {columns.map((column, colIndex) => (
                        <td
                          key={colIndex}
                          className="px-4 py-2 whitespace-nowrap text-sm text-cyber-text"
                        >
                          {column.type === 'number'
  ...
  // Los otros métodos de renderizado (renderAdvancedStep, renderConfirmStep) quedan igual que en tu implementación original
            ...
            <div>
              ...
              <p className="text-cyber-text">
                {uploadMethod === 'file' 
                  ...
                  : uploadMethod === 'api'
                  ? (connectionConfig?.connectionString || 'No connection')
                  : 'Data pasted'}
              </p>
            </div>
    ...
    <div className="fixed inset-0 z-50 overflow-auto bg-black/60 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-cyber-dark/90 border border-cyber-detail rounded-lg shadow-lg w-full max-w-3xl mx-4 overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-cyber-detail">
          ...
          <button 
            onClick={onClose}
            className="text-cyber-text/70 hover:text-cyber-text"
          >
            <X size={20} />
          </button>
        </div>
